...
export const register = async (req, res) => {
  try {
    ...
    if (!req.body.name || !req.body.lastname || !req.body.email || !req.body.password || !req.body.role) return res.status(400).json({
      message: 'A required field was left empty.'
    }); // 1. Encrpyt password

    const hashedPassword = await bcrypt.hash(req.body.password, 10); // 2. Create user
...
export const validateRefreshToken = async (req, res) => {
  try {
    ...
    if (refreshToken == null) return res.status(400).json({
    ...
    if (!tokenInDb) return res.status(400).json({
...
export const deleteRefreshToken = async (req, res) => {
  try {
    ...
    const deletedRows = await RefreshTokenModel.destroy({
      where: {
        token: refreshToken
      }
    });
    if (deletedRows < 1) return res.status(400).json({
      message: 'Refresh token not found or not deleted'
    });
    res.status(200).json({
